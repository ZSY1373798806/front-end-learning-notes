...
| 执行顺序 | 生命周期钩子        | 发生时机                                       | 核心作用（源码角度）                                         |
| -------- | ------------------- | ---------------------------------------------- | ------------------------------------------------------------ |
| 1        | **父 beforeCreate** | 父组件实例刚 new 出来，`data/props` 还没初始化 | 初始化一些内部状态（`_events`、`_watchers`），挂载父组件的依赖关系。此时 **data、props、methods 还不可用** |
| 2        | **父 created**      | 完成 data/props/methods/computed/watch 初始化  | 可以访问到响应式数据（`this.xxx`），可以进行数据请求、计算初始状态。此时 DOM 还没生成。 |
| 3        | **父 beforeMount**  | 即将执行首次渲染（`vm._render()`）             | 开始创建父组件的 render watcher，调用 `render()` 生成 VNode 树。遇到子组件节点 → 创建子组件 VNode，准备初始化子组件。 |
| 4        | **子 beforeCreate** | 子组件实例刚 new 出来                          | 初始化子组件内部生命周期状态、事件系统，但 props/data 还未注入。 |
| 5        | **子 created**      | 子组件 data/props 完成初始化                   | 可以访问子组件自己的数据，适合请求子组件内部所需数据、计算初始状态。 |
| 6        | **子 beforeMount**  | 子组件即将渲染                                 | 为子组件创建渲染 watcher，执行 `render()` 生成子组件 VNode 树，还没 patch DOM。 |
| 7        | **子 mounted**      | 子组件的 DOM 已经挂载到页面                    | 此时能通过 `this.$el` 获取真实 DOM，可以操作 DOM、注册定时器、监听外部事件。 |
| 8        | **父 mounted**      | 父组件渲染完成，包含所有子组件                 | 说明整个父组件 DOM 树（包括子组件）都挂载到页面，可以安全访问子组件 DOM/方法。 |

为什么 `父组件的beforeMount` 早于 `子组件beforeCreate` ?

- `mountComponent`函数一开始就调用`callHook(vm, 'beforeMount')`

- 然后才创建渲染Watcher并执行`updateComponent`

- 在`updateComponent`的渲染过程中才会创建和初始化子组件
  ...
  export function mountComponent (
    ...
    vm.$el = el
    // 1. 首先调用父组件的 beforeMount 钩子
    callHook(vm, 'beforeMount')
    let updateComponent
    updateComponent = () => {
      // 2. 然后执行渲染函数，这时才会创建子组件
      vm._update(vm._render(), hydrating)
    }
    // 3. 创建渲染Watcher，执行updateComponent
  ...
  Vue.prototype._render = function (): VNode {
    ...
    // 执行渲染函数，在这个过程中会遇到子组件标签
    // 并调用 createElement 创建子组件VNode
...
- 为每个绑定的节点生成更新函数，并且创建对应的 Watcher，（创建Watcher时会触发getter，进行依赖收集）。
...
- 步骤：
    ...
    <img src="https://i-coder.oss-cn-beijing.aliyuncs.com/files/20250905110810.png" style="zoom: 50%;" />
    
    <img src="https://i-coder.oss-cn-beijing.aliyuncs.com/files/20250904195832.png" style="zoom:33%;" />
    
    <img src="https://i-coder.oss-cn-beijing.aliyuncs.com/files/20250904200036.png" style="zoom:33%;" />
...
- Proxy
	...
	| 方法                                               | 对应操作符                | 说明                                                  |
	| -------------------------------------------------- | ------------------------- | ----------------------------------------------------- |
	| `Reflect.get(target, prop, receiver)`              | `obj.prop`                | 获取属性值，类似 `target[prop]`                       |
	| `Reflect.set(target, prop, value, receiver)`       | `obj.prop = value`        | 设置属性值，类似 `target[prop] = value`，返回是否成功 |
	| `Reflect.has(target, prop)`                        | `key in obj`              | 检查属性是否存在，类似 `prop in target`               |
	| `Reflect.deleteProperty(target, prop)`             | `delete obj.prop`         | 删除属性，类似 `delete target[prop]`                  |
	| `Reflect.ownKeys(target)`                          | `Object.keys()`           | 获取对象自身的所有属性（包括 Symbol）                 |
	| `Reflect.defineProperty(target, prop, descriptor)` | `Object.defineProperty()` | 定义属性，类似 `Object.defineProperty`                |
