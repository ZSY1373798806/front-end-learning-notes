...
- **模块解析**：递归解析入口文件的依赖（import/require），对每个模块执行：
...
| 类型        | 生成依据         | 变化条件            | 典型适用场景               |
| ----------- | ---------------- | ------------------- | -------------------------- |
| hash        | 整个项目构建结果 | 任何文件修改        | 几乎不用（开发环境临时用） |
| chunkhash   | 单个 chunk 内容  | 所属 chunk 内容修改 | JS 文件                    |
| contenthash | 单个文件自身内容 | 文件自身内容修改    | CSS、图片等资源文件        |
