...
1. **自动重连**：结合指数退避或固定间隔，避免频繁重连
2. **心跳检测**：保证连接活跃，及时发现断线
3. **事件分发**：统一管理 open、message、close、error
4. **日志管理**：开发环境打印，生产环境关闭
5. **消息解析**：心跳消息与业务消息分开处理
6. **React 集成**：组件卸载时关闭连接，避免内存泄漏
