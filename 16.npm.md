### 输入npm run xxx 命令后发生了什么

- **第1步：npm 启动和解析**

  - Shell 执行 `node npm-cli.js run dev`
...
- **第2步：项目定位和脚本查找**
  ...
  - 在 `scripts` 字段中找到 `dev: "vite"`

- **第3步：环境变量设置（核心！）**

  - **`path`注入**

    - 临时将 `node_modules/.bin` 目录添加到系统环境变量 `PATH` 的最前面

    - 假设系统 `PATH` 原本是 **`"/usr/bin:/bin"`**，注入后变为 **`"./node_modules/.bin:/usr/bin:/bin"`**，确保优先使用项目本地依赖

      ```bash
      PATH=./node_modules/.bin:$原PATH
      ```

  - **环境变量注入**（`process.env` 可读取）：

    - `npm_lifecycle_event=xxx`
    - `npm_package_*`（把 package.json 的字段扁平注入，如 `npm_package_name`）
    - `npm_config_*`（npm 配置）
...
- **第5步：可执行文件查找（你提到的）**
  ...
  - 在 `node_modules/.bin` 中查找 `vite`

  - `vite` 是软链接 → 指向 `../vite/bin/vite.js`

- **第6步：子进程创建**

  - 使用 `child_process.spawn()` 创建子进程

  - 在修改后的环境中执行：predev -> dev -> postdev
...
> "当执行 `npm run dev` 时，首先 npm 会在 package.json 中找到对应脚本，然后**关键是修改 PATH 环境变量**，将 `./node_modules/.bin` 添加到最前面。接着在 node_modules/.bin 中查找可执行文件，这些是软链接指向实际的 JS 文件。最后创建子进程在修改后的环境中执行命令。"

### npm run xxx & npx xxx & node xxx.js

| 维度                  | `npm run xxx`                                                | `npx xxx`                                       | `node xxx.js`                              |
| --------------------- | ------------------------------------------------------------ | ----------------------------------------------- | ------------------------------------------ |
| **执行对象**          | `package.json` 中 `scripts` 配置的命令                       | 依赖包中的可执行命令（如 `webpack`）            | 具体的 JS 文件（如 `app.js`）              |
| **依赖处理**          | 自动查找本地 `node_modules/.bin` 依赖                        | 优先本地依赖，无则临时安装（用完删除）          | 需手动确保依赖已安装，通过 `require` 引入  |
| **环境变量/路径注入** | 注入 `npm_lifecycle_event`、`npm_package_*` 等；自动 PATH 注入 `.bin` | 不注入 npm 生命周期变量；只做 `.bin` 或远程解析 | 没有额外环境变量；不会自动注入 `.bin` 路径 |
| **是否需预配置**      | 需在 `scripts` 中定义 `xxx`                                  | 无需配置，直接执行命令名                        | 无需配置，直接指定文件路径                 |
| **支持的语法**        | 支持 Shell 语法（`&&`等）                                    | 支持命令参数（如 `npx webpack --mode dev`）     | 仅支持 JS 代码（文件内逻辑）               |
| **核心场景**          | 项目固定流程（启动、构建、测试等）                           | 临时执行工具（如脚手架、代码检查）              | 直接运行单个 JS 文件（如脚本、服务）       |
| **典型用法**          | `npm run dev` `npm run build`                                | `npx webpack -v` `npx eslint src`               | `node server.js`                           |

### `package.json` 中的 `bin` 字段有什么用？如果自己开发一个 npm 包，想让用户全局安装后能直接执行命令（如 `my-cli init`），该怎么配置？

- **考察点**：npm 包的可执行命令配置（与 `node_modules/.bin` 软链接呼应）。
- 回答思路
  - `bin` 字段的作用：定义当前 npm 包的可执行命令映射，让 npm 安装包时（无论本地还是全局），自动创建对应软链接（本地在 `node_modules/.bin`，全局在系统 `PATH` 目录）。
  - 开发 CLI 包的配置步骤：
    1. 在包的 `package.json` 中配置 `bin`：`{"bin": {"my-cli": "bin/index.js"}}`（`my-cli` 是用户要执行的命令，`bin/index.js` 是实际入口文件）；
    2. 在入口文件（`bin/index.js`）头部加 Shebang 声明：`#!/usr/bin/env node`（告诉系统用 Node.js 执行该文件）；
    3. 本地测试：用 `npm link` 将包链接到全局，执行 `my-cli init` 验证；
    4. 发布后，用户全局安装 `npm i -g my-cli`，即可直接执行 `my-cli init`。
...
"scripts": {
    "prepare": "husky",
    "preinstall": "npx only-allow pnpm" // 强制使用`pnpm install`
},
```
...
- **版本符号**：`@` 用于指定版本 / 标签
  ...
  | 符号    | 匹配范围                                                     | 风险等级 | 适用场景                   |
  | ------- | ------------------------------------------------------------ | -------- | -------------------------- |
  | `^`     | 主版本固定，次版本和补丁自动更新（主版本为 0 时，次版本固定） | 中       | 开发阶段，信任依赖包的更新 |
  | `@`     | 精确锁定版本 / 标签 /commit                                  | 低       | 生产环境，测试预发布版本   |
  | `~`     | 主版本和次版本固定，补丁自动更新                             | 低       | 仅接受 Bug 修复            |
  | `>`/`<` | 自定义范围匹配                                               | 高       | 复杂版本约束（需谨慎使用） |
