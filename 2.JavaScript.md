...
- 优缺点
  ...
  - **解决**：手动释放引用，变量值设为null；不要在全局保存大量的闭包状态
...
- 方法存储位置
  ...
  class Person {
    ...
    sayHello() { // 普通函数绑定在构造函数原型(prototype)上
  ...
  - 实现
    ...
    - 自定义deepCopy
         ...
         function deepClone(target, map = new Map()) {
           ...
           // 2. 处理循环引用（若已拷贝过该对象，直接返回缓存的结果）
           if (map.has(target)) {
             return map.get(target);
           }
           // 3. 处理引用类型（数组/对象/日期/正则等）
           let cloneTarget;
           ...
           // 5. 递归拷贝属性（数组用 forEach，对象用 Object.keys）
           if (Array.isArray(target)) {
         ...
         // 测试（含循环引用）：
         const obj = { a: 1, b: { c: 2 } };
         obj.d = obj; // 循环引用（obj.d 指向 obj 自身）
         const cloneObj = deepClone(obj);
         console.log(cloneObj.a); // 1
         console.log(cloneObj.b.c); // 2
         console.log(cloneObj.d === cloneObj); // true（成功处理循环引用）
         ```
