...
#### single-spa 为什么依赖 UMD 格式？

single-spa 的核心需求是「动态加载子应用，并调用其 `bootstrap`/`mount`/`unmount` 生命周期函数」。要满足这个需求，需要以下 3 个关键场景：

##### 1. 兼容 single-spa 的「动态加载机制」

single-spa 加载子应用的核心逻辑是：

1. 通过模块加载器（如 SystemJS、import () 动态导入）加载子应用的打包产物（如 `app1.js`）；
2. 从加载结果中提取子应用暴露的 `bootstrap`/`mount`/`unmount` 函数；
3. 按生命周期调用这些函数（如路由匹配时调用 `mount`）。

若子应用不使用 UMD 格式，会出现「加载失败」或「无法提取生命周期函数」的问题：

- 若用 `libraryTarget: 'commonjs2'`（CommonJS 格式）：浏览器环境（无 Node.js 模块系统）无法解析 `module.exports`，加载后会得到 `undefined`；
- 若用 `libraryTarget: 'amd'`（AMD 格式）：需依赖 RequireJS 等 AMD 加载器，而 single-spa 默认使用的 SystemJS 对 AMD 兼容性有限；
- 若用 `libraryTarget: 'var'`（全局变量格式）：虽能在浏览器全局暴露，但动态加载时（如 SystemJS 加载）无法通过「模块导出」获取函数，只能手动从 `window` 上取，易出现命名冲突且不符合模块化规范。

而 UMD 格式能让模块加载器（如 SystemJS）直接解析出子应用的导出对象，single-spa 可轻松提取 `bootstrap`/`mount`/`unmount`：

```javascript
// single-spa 加载 UMD 格式子应用的逻辑（简化）
System.import('app1').then((app) => {
  // app 是 UMD 格式导出的对象，直接包含生命周期函数
  const { bootstrap, mount, unmount } = app;
  // 调用生命周期
  bootstrap().then(() => mount(props));
});
```

##### 2. 支持子应用「独立运行」（开发调试必备）

子应用在开发阶段通常需要「独立启动调试」（如 `npm run serve` 单独打开 app1），而无需依赖 single-spa 基座。UMD 格式能完美支持这种场景：

- 独立运行时：子应用打包为 UMD 格式后，会自动作为全局变量暴露到 `window` 上（如 `window.app1`），此时子应用可直接执行 `mount` 函数渲染自身；
- 接入基座时：又能被 single-spa 作为模块加载，无需修改打包配置。

例如 Vue 子应用独立运行的逻辑（依赖 UMD 全局暴露）：

```javascript
// 子应用 main.js（独立运行判断）
if (!window.singleSpaNavigate) {
  // 独立运行时，从 window 上获取 UMD 导出的 mount 函数
  window.app1.mount({ container: '#app' });
}
```

##### 3. 避免「模块系统冲突」（跨团队协作保障）

single-spa 架构下，子应用可能由不同团队开发，使用不同的技术栈（Vue/React/Angular）和模块工具（Webpack/Vite/Rollup）。若子应用导出格式不统一（如 A 用 CommonJS，B 用 AMD），会导致基座加载逻辑复杂，甚至出现冲突。

UMD 作为「通用格式」，可作为跨团队的「模块导出标准」—— 无论子应用用什么工具打包，只要输出 UMD 格式，single-spa 就能统一加载，降低协作成本。

#### 配置 UMD 时的「配套要求」

仅配置 `libraryTarget: 'umd'` 不够，还需配合 `library` 配置指定「模块名称」，确保 UMD 格式能正确暴露子应用：

```javascript
// 子应用 webpack.config.js（关键配置）
module.exports = {
  output: {
    filename: 'js/app.js',
    publicPath: process.env.REACT_APP_PUBLIC_URL,
    library: 'app1', // 模块名称（独立运行时暴露为 window.app1，加载时作为模块标识）
    libraryTarget: 'umd', // 核心：输出 UMD 格式
    globalObject: 'this' // 修复 UMD 在浏览器/Node 环境的兼容性（避免 window 未定义）
  }
};
```

- `library: 'app1'`：定义 UMD 模块的名称，确保独立运行时能通过 `window.app1` 访问，动态加载时能被模块加载器正确识别；
- `globalObject: 'this'`：解决 UMD 格式在 Node.js 环境（`global`）和浏览器环境（`window`）的全局对象差异，避免打包后出现 `window is not defined` 错误。

#### 总结：UMD 格式的核心价值

single-spa 子应用配置 `libraryTarget: 'umd'`，本质是为了满足「**一套打包产物，适配两种核心场景**」：

1. **接入基座场景**：能被 single-spa 通过模块加载器（如 SystemJS）动态加载，并正确提取生命周期函数；
2. **独立运行场景**：能作为全局变量暴露，支持开发阶段单独调试，无需修改配置。

若不使用 UMD 格式，子应用要么无法被 single-spa 加载，要么无法独立运行，因此 `libraryTarget: 'umd'` 是 single-spa 子应用的「必选配置」。
...
- ##### 挂载远程模块的方法
   ...
   */
  export const loadRemoteVue = (
      compName: VueComponent,
...
### 如何远程消费（consumer非webpack5的情况）

#### MF1.0

当远程应用（remote）构建时，Webpack 会输出一个 `remoteEntry.js`。
 这个文件在加载后，会在全局挂载一个容器对象，例如：

```js
window["ui"] = {
  get: (module) => { ... },  // 获取远程暴露的模块工厂函数
  init: (shareScope) => { ... } // 初始化共享依赖作用域
}
```

主应用（host）在运行时就可以通过 `window.ui.get("./xxx")` 来拿到远程模块。

```js
export const getRemoteScript = async (
  remoteUrl: string, // 远程地址（remoteEntry.js）
  mfName: string,    // 远程容器名字，比如 "ui"
  module: string     // 暴露的模块，比如 "./workbench"
): Promise<any> => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      // 2. 检查远程容器是否已存在
      let container: any = (window as any)[mfName];
      if (!container) {
        // 3. 动态插入 <script>
        const script = document.createElement("script");
        ...
        script.onload = async () => {
          container = (window as any)[mfName];
          if (!container) {
            reject(new Error(`未找到远程容器: ${mfName}`));
            return;
          }
          // 4. 初始化远程容器的共享作用域
          try {
            ...
            console.warn(`[MF] 容器 ${mfName} init 失败:`, e);
          }
          // 5. 获取远程模块工厂函数
          const factory = await container.get(module);
        ...
        script.onerror = () => {
          reject(new Error(`加载远程地址失败: ${remoteUrl}`));
        };
...
await init({
  name: "host",
  remotes: {
    ui: "https://cdn.example.com/ui/mf-manifest.json"
  }
});
```
...
```js
const Button = await loadRemote("ui/Button");
```

这里的 `loadRemote` 会自动：

1. 下载远程 manifest
2. 解析 `entry` 并拉取对应 chunk
3. 处理 shared scope（和本地依赖对齐）
4. 返回模块

#### 🧩 总结

| 特性             | MF 1.0 (`remoteEntry.js`)      | MF 2.0 (`mf-manifest.json` + runtime) |
| ---------------- | ------------------------------ | ------------------------------------- |
| **运行时挂载**   | `window["ui"] = { get, init }` | runtime SDK 内部管理                  |
| **构建工具依赖** | Webpack 专属                   | Webpack / Vite / Rspack / RN          |
| **元信息**       | 只暴露模块路径                 | 模块 + 依赖 + 版本 + 优化提示         |
| **动态发现**     | 不支持                         | 原生支持                              |
| **调用方式**     | `await window.ui.get("./x")`   | `await loadRemote("ui/x")`            |
